...
export function validatePluginManifest(raw: Record<string, unknown>): ValidatedPluginManifest {
  ...
  for (const field of required) {
    if (!raw[field] || typeof raw[field] !== "string") {
      throw Object.assign(
        new Error(`manifest.json: field "${field}" is missing or invalid.`),
        { status: 400 }
      );
    }
  }
  ...
  if (!SEMVER_RE.test(version)) {
    throw Object.assign(
      new Error(`manifest.json: version must follow semver (major.minor.patch); received "${version}".`),
      { status: 400 }
    );
  }
  ...
  if (!SLUG_RE.test(slug)) {
    throw Object.assign(
      new Error(`manifest.json: the slug "${slug}" is invalid (use lowercase letters, numbers, and hyphens).`),
      { status: 400 }
    );
  }
  ...
  for (const item of contributions.sidebar) {
    if (!declaredPages.has(item.page)) {
      throw Object.assign(
        new Error(`manifest.json: the menu item "${item.label}" points to the page "${item.page}", which is not declared in contributions.pages.`),
        { status: 400 }
      );
    }
  }
