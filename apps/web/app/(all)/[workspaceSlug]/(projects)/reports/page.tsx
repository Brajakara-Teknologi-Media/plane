    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-2">
          <h3 className="text-13 font-semibold text-primary">{t(report.title)}</h3>
          <ChevronRight className="h-4 w-4 shrink-0 text-tertiary transition-transform group-hover:translate-x-0.5" />
        </div>
        <p className="mt-1 text-12 leading-snug text-secondary">{t(report.description)}</p>
      </div>
    </button>
  );
}
...
function ReportsLandingPage() {
  ...
  const pageTitle = currentWorkspace?.name
    ? `${currentWorkspace.name} - ${t("reports.page_title")}`
  ...
  return (
    <div className="flex h-full w-full flex-col overflow-hidden">
      ...
      <div className="border-b border-subtle px-6 py-4">
        <h1 className="text-lg font-semibold">{t("reports.page_title")}</h1>
        <p className="text-13 text-secondary">{t("reports.page_description")}</p>
      </div>
      ...
      <div className="flex-1 space-y-8 overflow-y-auto px-6 py-6">
        {REPORT_CATEGORIES.map((cat) => {
          ...
          return (
            <section key={cat.key}>
              <div className="mb-3">
                <h2 className="text-14 font-semibold text-primary">{t(cat.label)}</h2>
                <p className="text-12 text-tertiary">{t(cat.description)}</p>
              </div>
