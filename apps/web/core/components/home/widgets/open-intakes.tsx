  ...
  async listOpenIntakes(slug: string, projectIds: string[]) {
    ...
    await Promise.all(
      projectIds.slice(0, 5).map(async (pid) => {
        const data = await this.get(`/api/workspaces/${slug}/projects/${pid}/inbox-issues/`, {
    ...
    return results.sort(
      (a, b) => new Date(b.issue?.created_at ?? 0).getTime() - new Date(a.issue?.created_at ?? 0).getTime()
    );
  }
}
...
export function OpenIntakesWidget({ workspaceSlug }: THomeWidgetProps) {
  ...
  useEffect(() => {
    ...
    intakeService
      .listOpenIntakes(workspaceSlug, joinedProjectIds)
      .then(setIntakes)
    ...
    <div className="rounded-xl border border-subtle bg-surface-1 p-4">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-13 font-semibold">Open requests</span>
          {intakes.length > 0 && (
      ...
      {intakes.length === 0 && <p className="py-4 text-center text-13 text-secondary">No open requests.</p>}
      <div className="space-y-2">
        {intakes.slice(0, 6).map((intake) => (
          ...
          >
            <span className="flex-1 truncate text-13">{intake.issue?.name ?? "Untitled"}</span>
