...
export const usePowerKModuleContextBasedActions = (): TPowerKCommandConfig[] => {
  ...
  const isEditingAllowed =
    allowPermissions(PROJECT_WORK_ROLES, EUserPermissionsLevel.PROJECT) && !moduleDetails?.archived_at;
  // translation
      ...
      await updateModuleDetails(workspaceSlug.toString(), projectId.toString(), moduleDetails.id, formData).catch(
        () => {
          setToast({
            ...
            message: "Could not update module. Please try again.",
          });
        }
      );
    },
  ...
  const toggleFavorite = useCallback(() => {
    ...
    try {
      ...
      setToast({
        type: TOAST_TYPE.ERROR,
        title: t("common.something_went_wrong_please_try_again"),
      });
    }
