    ...
    <div ref={dropdownRef} className="relative h-11 border-b-[0.5px] border-subtle">
      <button
        ...
        className={cn(
          ...
          "group-[.selected-issue-row]:bg-accent-primary/5 hover:bg-layer-1",
      ...
      >
        ...
        {selectedEntity ? (
          <>
            ...
            {!disabled && (
              <span
                ...
                className="shrink-0 rounded p-px hover:bg-layer-2"
              >
          ...
          <span className="text-placeholder">Entity</span>
        )}
      </button>

      {open && (
        <div className="shadow-lg absolute top-full left-0 z-200 mt-0.5 w-56 overflow-hidden rounded-md border border-strong bg-surface-1">
          ...
          <div className="max-h-48 overflow-y-auto py-1">
            {loading && <p className="px-3 py-2 text-caption-sm-regular text-secondary">{t("common.loading")}</p>}
            {!loading && filtered.length === 0 && (
              <p className="px-3 py-2 text-caption-sm-regular text-secondary">{t("common.none_found")}</p>
            )}
            {!loading && entityId && (
              <button
                type="button"
